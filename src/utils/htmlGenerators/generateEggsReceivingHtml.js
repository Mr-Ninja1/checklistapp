...
function generateEggsReceivingHtml(payload = {}, options = {}) {
  const p = payload.payload || payload;
  const rows = Array.isArray(p.formData) ? p.formData : (p.rows || []);
  const meta = p.metadata || {};
  const title = escapeHtml(p.title || p.name || 'EGGS RECEIVING FORM');
  ...
  const ths = `<th>Product</th><th>Supplier</th><th>Pallet/Carton</th><th>Qty</th><th>Condition</th><th>Remarks</th>`;

  const rowsHtml = (rows.length ? rows : []).map(r => `<tr><td>${renderSimple(r.product||r.name||'')}</td><td>${renderSimple(r.supplier||'')}</td><td>${renderSimple(r.pallet||r.carton||'')}</td><td>${renderSimple(r.quantity||'')}</td><td>${renderSimple(r.condition||'')}</td><td>${renderSimple(r.remarks||'')}</td></tr>`).join('');
...
module.exports = generateEggsReceivingHtml;
