...
const BASE_DRAFT_KEY = 'display_chiller_temperature_log';
const MAX_DAYS = 31;
...
const initialMeta = {
  subject: 'DISPLAY CHILLER TEMPERATURE LOG SHEET - UPRIGHT',
  issueDate: '',
  companyName: 'Bravo',
  ...
  location: '',
  displayChillerName: 'Upright',
  hseqManagerSign: '',
  complexManagerSign: '',
  fscSign: '',
};

export default function DisplayChillerTemperatureLog_Upright(props = {}) {
  ...
  useEffect(() => {
    // If a wrapper provided a default display chiller name, apply it on first mount
    if (props.defaultDisplayChillerName && meta.displayChillerName !== props.defaultDisplayChillerName) {
      setMeta(prev => ({ ...prev, displayChillerName: props.defaultDisplayChillerName }));
    }
  ...
  const instructionText = (props.formType === 'DisplayChillerTemperatureLog_Gelato' || props.defaultDisplayChillerName === 'Gelato' || meta.displayChillerName === 'Gelato')
    ? 'The temperature of the freezer should not be below -2°C.'
    : 'The temperature of the chiller should be between 0°C and 4°C.';
  ...
  const handleSubmit = async () => {
    ...
    setBusy(true);
    try {
      const metadataForPayload = { ...meta, displayChillerName: meta.displayChillerName || meta.subject };
      const formType = props.formType || 'DisplayChillerTemperatureLog_Upright';
      const title = props.title || 'DISPLAY CHILLER TEMPERATURE LOG SHEET - Upright';
      const payload = {
        formType,
  ...
  const handleSaveDraft = async () => {
    setBusy(true);
    try { const draftKey = props.draftKey || `${BASE_DRAFT_KEY}_upright_draft`; await setDraft(draftKey, { rows, meta }); } catch (e) { console.warn('save draft error', e); }
    setBusy(false);
  };
      ...
      <EditableFormContainer editMode={editMode} setEditMode={setEditMode} onSaveDraft={handleSaveDraft} actionButtons={actionButtons}>
        ...
        <View style={styles.metaContainer}>
          ...
          <View style={styles.metaBottomRow}>
            <View style={[styles.metaBottomItem, { flex: 3 }]}> 
              <Text style={styles.metaBold}>Name of Display Chiller:</Text>
              {editMode ? (
                <TextInput
                  style={[styles.monthlyInput, { width: '100%' }]}
                  value={meta.displayChillerName}
                  onChangeText={v => setMetaField('displayChillerName', v)}
                  placeholder="e.g., Upright"
                  editable={editMode}
                />
              ) : (
                  <Text style={styles.readOnlyMeta}>{meta.displayChillerName}</Text>
              )}
            </View>
        ...
        <View style={styles.monthlyInfo}>
            ...
            <View style={styles.monthlyInputRow}>
              ...
              {editMode ? <TextInput style={[styles.monthlyInput, { flex: 3 }]} value={meta.location} onChangeText={v => setMetaField('location', v)} placeholder="e.g., Front Counter" editable={editMode} /> : <Text style={[styles.readOnlyMeta, { flex: 3 }]}>{meta.location}</Text>}
            </View>
        </View>

        <View style={styles.instructionBox}>
          <Text style={styles.instructionText}><Text style={{ fontWeight: '900' }}>Instruction:</Text> {instructionText}</Text>
        </View>
        ...
        <View style={styles.tableWrap}>
          ...
          {rows.map((row, ri) => (
            <View key={ri} style={styles.row}>
              ...
              <View style={[styles.cell, { flex: COL_FLEX.COMPLEX_SIGN }]}>
                {editMode ? (
                  <SignatureField value={row.complexManagerSign} onChange={(v) => setCell(ri, 'complexManagerSign', v)} editable={editMode} width={160} height={44} placeholder="Complex Sign" />
                ) : (() => {
                  const v = row.complexManagerSign;
                  const uri = v ? (String(v).startsWith('data:') ? v : `data:image/png;base64,${v}`) : null;
                  return uri ? <SignatureThumb uri={uri} width={160} height={44} layers={6} spread={0.9} /> : <Text style={styles.readOnlyCell}>{v || ''}</Text>;
                })()}
              </View>

              <View style={[styles.cell, { flex: COL_FLEX.FSC_SIGN }]}>
                {editMode ? (
                  <SignatureField value={row.fscSign} onChange={(v) => setCell(ri, 'fscSign', v)} editable={editMode} width={140} height={44} placeholder="FSC Sign" />
                ) : (() => {
                  const v = row.fscSign;
                  const uri = v ? (String(v).startsWith('data:') ? v : `data:image/png;base64,${v}`) : null;
                  return uri ? <SignatureThumb uri={uri} width={140} height={44} layers={6} spread={0.9} /> : <Text style={styles.readOnlyCell}>{v || ''}</Text>;
                })()}
              </View>

              <View style={[styles.cell, { flex: COL_FLEX.HSEQ_SIGN }]}>
                {editMode ? (
                  <SignatureField value={row.hseqManagerSign} onChange={(v) => setCell(ri, 'hseqManagerSign', v)} editable={editMode} width={160} height={44} placeholder="HSEQ Sign" />
                ) : (() => {
                  const v = row.hseqManagerSign;
                  const uri = v ? (String(v).startsWith('data:') ? v : `data:image/png;base64,${v}`) : null;
                  return uri ? <SignatureThumb uri={uri} width={160} height={44} layers={6} spread={0.9} /> : <Text style={styles.readOnlyCell}>{v || ''}</Text>;
                })()}
              </View>
