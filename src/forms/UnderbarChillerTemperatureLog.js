...
const DRAFT_KEY = 'kitchen_underbar_chiller_temperature_log_draft_1';
const MAX_DAYS = 31;
...
export default function UnderbarChillerTemperatureLog() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        title: 'Kitchen - Underbar Chiller Temperature Log 1',
      ...
      // CHANGES MADE HERE:
      // 1. Removed removeDraft(DRAFT_KEY) to keep the draft in local storage.
      // 2. Removed the setRows(initialRows) and setMeta reset logic to keep UI populated.
  ...
  const GROUP_FLEX = {
    MORNING: COL_FLEX.TEMP + COL_FLEX.SIGN,
    AFTERNOON: COL_FLEX.TEMP + COL_FLEX.SIGN,
    EVENING: COL_FLEX.TEMP + COL_FLEX.SIGN,
  };
      ...
      <EditableFormContainer editMode={editMode} setEditMode={setEditMode} onSaveDraft={handleSaveDraft} actionButtons={actionButtons}>
        ...
        <View style={styles.metaContainer}>
          ...
          <View style={styles.metaBottomRow}>
            <View style={[styles.metaBottomItem, { flex: 3 }]}>
              ...
              {editMode ? <TextInput style={styles.monthlyInput} value={meta.subject} onChangeText={v => setMetaField('subject', v)} /> : <Text style={styles.readOnlyMeta}>{meta.subject}</Text>}
            </View>
            <View style={styles.metaBottomItem}>
              <Text style={styles.metaBold}>Compiled By:</Text>
              {editMode ? <TextInput style={styles.monthlyInput} value={meta.compiledBy} onChangeText={v => setMetaField('compiledBy', v)} /> : <Text style={styles.readOnlyMeta}>{meta.compiledBy}</Text>}
            </View>
            <View style={styles.metaBottomItem}>
              <Text style={styles.metaBold}>Approved By:</Text>
              {editMode ? <TextInput style={styles.monthlyInput} value={meta.approvedBy} onChangeText={v => setMetaField('approvedBy', v)} /> : <Text style={styles.readOnlyMeta}>{meta.approvedBy}</Text>}
            </View>
          </View>
        </View>
        ...
        <View style={styles.monthlyInfo}>
            <View style={styles.monthlyInputRow}>
              ...
              {editMode ? <TextInput style={styles.monthlyInput} value={meta.month} onChangeText={v => setMetaField('month', v)} placeholder="e.g., October" /> : <Text style={styles.readOnlyMeta}>{meta.month}</Text>}
            </View>
            <View style={styles.monthlyInputRow}>
              <Text style={styles.monthlyLabel}>Year:</Text>
              {editMode ? <TextInput style={styles.monthlyInput} value={meta.year} onChangeText={v => setMetaField('year', v)} placeholder="e.g., 2025" /> : <Text style={styles.readOnlyMeta}>{meta.year}</Text>}
            </View>
            <View style={styles.monthlyInputRow}>
              <Text style={styles.monthlyLabel}>Location:</Text>
              {editMode ? <TextInput style={[styles.monthlyInput, { flex: 3 }]} value={meta.location} onChangeText={v => setMetaField('location', v)} placeholder="e.g., Underbar Chiller" /> : <Text style={[styles.readOnlyMeta, { flex: 3 }]}>{meta.location}</Text>}
            </View>
        </View>
        ...
        <View style={styles.tableWrap}>
          ...
          {rows.map((row, ri) => (
            <View key={ri} style={styles.row}>
              ...
              <View style={[styles.cell, styles.borderRight, { flex: COL_FLEX.SIGN }]}>{editMode ? <SignatureField value={row.staffSignMorning} onChange={(v) => setCell(ri, 'staffSignMorning', v)} editable={true} width={140} height={40} /> : (row.staffSignMorning ? <SignatureThumb uri={row.staffSignMorning.startsWith('data:') ? row.staffSignMorning : `data:image/png;base64,${row.staffSignMorning}`} width={140} height={40} /> : <Text style={styles.readOnlyCell}>-</Text>)}</View>
              ...
              <View style={[styles.cell, styles.borderRight, { flex: COL_FLEX.SIGN }]}>{editMode ? <SignatureField value={row.staffSignAfternoon} onChange={(v) => setCell(ri, 'staffSignAfternoon', v)} editable={true} width={140} height={40} /> : (row.staffSignAfternoon ? <SignatureThumb uri={row.staffSignAfternoon.startsWith('data:') ? row.staffSignAfternoon : `data:image/png;base64,${row.staffSignAfternoon}`} width={140} height={40} /> : <Text style={styles.readOnlyCell}>-</Text>)}</View>
              ...
              <View style={[styles.cell, styles.borderRight, { flex: COL_FLEX.SIGN }]}>{editMode ? <SignatureField value={row.staffSignEvening} onChange={(v) => setCell(ri, 'staffSignEvening', v)} editable={true} width={140} height={40} /> : (row.staffSignEvening ? <SignatureThumb uri={row.staffSignEvening.startsWith('data:') ? row.staffSignEvening : `data:image/png;base64,${row.staffSignEvening}`} width={140} height={40} /> : <Text style={styles.readOnlyCell}>-</Text>)}</View>

              <View style={[styles.cell, styles.borderRight, { flex: COL_FLEX.CORRECTIVE_ACTION }]}>{editMode ? <TextInput style={styles.input} value={row.outOfSpecAction} onChangeText={v => setCell(ri, 'outOfSpecAction', v)} placeholder="Action" /> : <Text style={styles.readOnlyCell}>{row.outOfSpecAction}</Text>}</View>
              <View style={[styles.cell, { flex: COL_FLEX.SUP_NAME_SIGN }]}>{editMode ? <SignatureField value={row.supNameSign} onChange={(v) => setCell(ri, 'supNameSign', v)} editable={true} width={220} height={44} /> : (row.supNameSign ? <SignatureThumb uri={row.supNameSign.startsWith('data:') ? row.supNameSign : `data:image/png;base64,${row.supNameSign}`} width={220} height={44} /> : <Text style={styles.readOnlyCell}>-</Text>)}</View>
              <View style={[styles.cell, { flex: COL_FLEX.COMPLEX_SIGN }]}>{editMode ? <SignatureField value={row.complexManagerSign} onChange={(v) => setCell(ri, 'complexManagerSign', v)} editable={true} width={160} height={44} /> : (row.complexManagerSign ? <SignatureThumb uri={row.complexManagerSign.startsWith('data:') ? row.complexManagerSign : `data:image/png;base64,${row.complexManagerSign}`} width={160} height={44} /> : <Text style={styles.readOnlyCell}>-</Text>)}</View>
              <View style={[styles.cell, { flex: COL_FLEX.FSC_SIGN }]}>{editMode ? <SignatureField value={row.fscSign} onChange={(v) => setCell(ri, 'fscSign', v)} editable={true} width={140} height={44} /> : (row.fscSign ? <SignatureThumb uri={row.fscSign.startsWith('data:') ? row.fscSign : `data:image/png;base64,${row.fscSign}`} width={140} height={44} /> : <Text style={styles.readOnlyCell}>-</Text>)}</View>
              <View style={[styles.cell, { flex: COL_FLEX.HSEQ_SIGN }]}>{editMode ? <SignatureField value={row.hseqManagerSign} onChange={(v) => setCell(ri, 'hseqManagerSign', v)} editable={true} width={160} height={44} /> : (row.hseqManagerSign ? <SignatureThumb uri={row.hseqManagerSign.startsWith('data:') ? row.hseqManagerSign : `data:image/png;base64,${row.hseqManagerSign}`} width={160} height={44} /> : <Text style={styles.readOnlyCell}>-</Text>)}</View>
            </View>
          ))}
        </View>
