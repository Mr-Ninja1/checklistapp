...
// AM slots: 06:00 to 15:00
const TIME_SLOTS = [
  '06:00AM','07:00AM','08:00AM','09:00AM','10:00AM','11:00AM','12:00PM','13:00PM','14:00PM','15:00PM'
];
...
export default function Bakery_SanitizingLog_AM() {
  ...
  const draftKey = 'bakery_sanitizing_log_am_draft';
  ...
  const buildPayload = (status = 'draft') => {
    ...
    // normalize signatures in rows and footer before returning payload
    ...
    return {
      formType: 'Bakery_SanitizingLog_AM',
      templateVersion: 'v1',
      title: 'Food Contact Surface Cleaning and Sanitizing Log Sheet - Bakery — AM',
      date: metadata.date,
  ...
  const handleSaveDraftClicked = async () => {
    ...
    try {
      ...
      try { console.info('Bakery_AM: draft payload', buildPayload('draft')); } catch (e) {}
      const res = await handleSaveDraft();
      try { console.info('Bakery_AM: draft saved result', res); } catch (e) {}
      Alert.alert('Draft saved', 'Draft was saved successfully');
    } catch (e) {
      console.error('Bakery_AM: save draft failed', e);
  ...
  return (
    <EditableFormContainer editMode={editMode} setEditMode={setEditMode} onSaveDraft={handleSaveDraft}>
      ...
      <View style={styles.headerRow}>
        ...
        <Text style={[styles.title, { fontSize: ms(16), flex: 1, textAlign: 'center' }]}>FOOD CONTACT SURFACE CLEANING AND SANITIZING LOG SHEET - BAKERY — AM</Text>
