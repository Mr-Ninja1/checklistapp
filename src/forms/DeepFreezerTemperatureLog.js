...
const initialMeta = {
  subject: 'DEEP FREEZER TEMPERATURE LOG SHEET',
  issueDate: '',
  companyName: 'Bravo',
...
export default function DeepFreezerTemperatureLog(props = {}) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Try to save via formStorage (adds history and handles upload queue). Fallback to addFormHistory.
      try {
        const formId = `DeepFreezerTemperatureLog_${Date.now()}`;
        await formStorage.saveForm(formId, payload);
      } catch (e) {
        // fallback: register history entry only
      ...
      <EditableFormContainer editMode={editMode} setEditMode={setEditMode} onSaveDraft={handleSaveDraft} actionButtons={actionButtons}>
        ...
        <View style={styles.metaContainer}>
          ...
          <View style={styles.metaBottomRow}>
            <View style={[styles.metaBottomItem, { flex: 3 }]}> 
              <Text style={styles.metaBold}>Name of Freezer:</Text>
              {editMode ? (
                <TextInput
                  style={[styles.monthlyInput, { width: '100%' }]}
                    value={meta.freezerName}
                    onChangeText={v => setMetaField('freezerName', v)}
                  placeholder="e.g., Blast Freezer"
                  editable={editMode}
                />
              ) : (
                  <Text style={styles.readOnlyMeta}>{meta.freezerName}</Text>
              )}
            </View>
        ...
        <View style={styles.monthlyInfo}>
            ...
            <View style={styles.monthlyInputRow}>
              ...
              {editMode ? <TextInput style={[styles.monthlyInput, { flex: 3 }]} value={meta.location} onChangeText={v => setMetaField('location', v)} placeholder="e.g., Deep Freezer" editable={editMode} /> : <Text style={[styles.readOnlyMeta, { flex: 3 }]}>{meta.location}</Text>}
            </View>
            {/* Name of Freezer is captured in the subject field above (meta.subject). Duplicate field removed. */}
        </View>

        <View style={styles.instructionBox}>
          <Text style={styles.instructionText}><Text style={{ fontWeight: '900' }}>Instruction:</Text> The temperature of the Deep Freezer should be less than -18°C and not fall below -12°C.</Text>
        </View>
